...
结论：当前环境有小红书、抖音、B站和微博完成研究全链路，状态为 **4/7 PASS**。其中抖音必须改用 Playwright Chromium 标准模式并人工完成登录；按本节严格标准，快手、贴吧和知乎不能写成“全链路正常”。知乎的后续深度文本补测见下一节。

### 3.1 深度文本补测（同日后续）

在后续“高阶玩法/策划分析”纠错中，知乎使用 Playwright Chromium 标准模式、人工扫码和查询 `原神 战斗系统 设计`，成功落盘 17 条结果；其中 12 篇为 2657—22352 字的完整回答/文章正文，标准化为 117 个带段落 ID、原始行号和 SHA256 的 L0 片段，5 条 34—485 字摘要被拒绝。修复搜索上限后，以 `原神 元素反应 设计` 实测 `crawler_max_notes_count=5` 精确落盘 5 条，保存登录态可复用。

随后对专栏文章 `614601101` 跑指定详情，正文和 20 条一级评论均成功落盘；这次运行同时暴露出评论上限没有传入客户端的问题。修复后单元测试确认 `max_count=1` 时只回传 1 条，但两次线上复验分别被页面导航超时，以及 `pong` 请求超时后误转旧二维码选择器阻断。搜索场景也没有重新补跑评论。

因此知乎当前记为 **PARTIAL-DEEP-TEXT**：登录、关键词搜索、全文落盘、指定详情正文/评论和 L0 分段都各自有成功记录，但修复后的严格限额尚未完成同一次线上闭环，故严格全链路总数仍是 **4/7 PASS**，不能将本节写成 5/7。
...
后续已在 API 返回 `bvid` 时优先保存标准 BV URL；旧 JSONL 的 `av...` 仍可交给新增的 B站 L0 脚本解析。
...
4. 贴吧更新百度安全验证后的二维码/登录选择器；知乎修复 `pong` 超时时的登录误判和旧二维码选择器，再以严格限额补跑搜索评论与指定详情。
5. B站标准 BV URL 已修复；仍可让详情解析器直接接受旧 `av...` URL。
6. 知乎搜索已严格截断到 `CRAWLER_MAX_NOTES_COUNT`；其余平台继续分别修复。
